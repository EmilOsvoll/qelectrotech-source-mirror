...
void ProjectMainConfigPage::readValuesFromProject()
{
	...
	QStringList required_attributes = {"customer", "projectfile", "projectno", "projectrev"};
...
void ProjectAutoNumConfigPage::initWidgets()
{
	...
	tab_widget->addTab(m_saw_conductor, tr("Conductors"));
	
		//Element tab
	m_saw_element = new SelectAutonumW(0);
	tab_widget->addTab(m_saw_element, tr("Elements"));
...
void ProjectAutoNumConfigPage::saveContextElement()
{
	...
	if (m_saw_element->contextComboBox()->currentText() == tr("Name of the new numbering"))
	{
		...
		m_saw_element->contextComboBox()->addItem(tr("Untitled"));
	}
...
void ProjectAutoNumConfigPage::saveContextConductor()
{
	...
	{
		m_project->addConductorAutoNum (tr("Untitled"), m_saw_conductor -> toNumContext());
		project()->setCurrentConductorAutoNum(tr("Untitled"));
		m_saw_conductor->contextComboBox()-> addItem(tr("Untitled"));
	}
	// If the text isn't yet to the autonum of the project, add this new item to the combo box.
...
void ProjectAutoNumConfigPage::saveContextFolio()
{
	...
	if (m_saw_folio->contextComboBox() -> currentText() == tr("Name of the new numbering")) {
		m_project->addFolioAutoNum (tr("Untitled"), m_saw_folio -> toNumContext());
		m_saw_folio->contextComboBox() -> addItem(tr("Untitled"));
	}
	// If the text isn't yet to the autonum of the project, add this new item to the combo box.
