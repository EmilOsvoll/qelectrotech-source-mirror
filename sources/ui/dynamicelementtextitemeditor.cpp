...
void DynamicElementTextItemEditor::apply()
{
	...
	if(!undo_list.isEmpty() && m_element->diagram())
	{
		...
		else
		{
			...
			us.beginMacro(tr("Edit element text"));
...
QUndoCommand *DynamicElementTextItemEditor::associatedUndo() const
{
	...
	if(parent_undo->childCount() >= 1)
	{
		if(parent_undo->childCount() >= 2)
			parent_undo->setText(tr("Edit %1 telement text").arg(QString::number(parent_undo->childCount())));
		return parent_undo;
	}
	else
		return nullptr;
...
{
	QString name = QInputDialog::getText(this, tr("Group name"), tr("Enter the name of the new group"));
